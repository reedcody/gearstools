...
ImageCollection.extract <- function(
		...
		stack_format, #options: RasterList, vrt, GDALdrivers
		RasterStacks_names, # change name of output??
		overwrite=F,
		verbose=F,
		# Vector stuff: 
		extract.sp = NULL, # default to filterBounds if NULL
		extract.raster = NULL,
		qa.mask = F,
		# Parallel options:
...
#### Image stuff #### 
	#foreach image in the ImageCollection (parallel)
...
if(!is.null(extract.sp)){
  ...
  foreach(i=seq(length(ImageCollection$Images))
          #,.packages="GEARStools"
  ) %do%
  {
   list.files(ImageCollection$Images[[i]]$metadata$decompressed_dir)
    foreach(i=seq(length(ImageCollection$Images)))
    #,.packages="GEARStools"
  ) %do% #change to %dopar% when GEARStools package loaded
  {
    tempimage<-Image(fname=ImageCollection$Images[[i]]$metadata$fname,driver=ImageCollection$Images[[i]]$metadata$driver,retrieve_metadata=F,retrieve_stack=retrieve_stack,stack_format=stack_format,
    ...
    lapply(tempimage$RasterStacks, function(x){
      extract(x,extract.sp_read) #check extract parameters
      #figure out dataframe list formatting
    }
  }
  }
}
