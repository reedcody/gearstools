...
Image.metadata.TM_Landsat_4_sr <- function(fname,decompressed_dir=tempdir(),return_raw=T,extras=NULL,verbose=F)
{
	driver="TM_Landsat_4_sr"
...
#	metadata$proj <- paste("+proj=utm +zone=",utm_zone," ",south,"+ellps=WGS84 +datum=WGS84 +units=m +no_defs",sep="")
	...
	ymax <- min(as.numeric(c(xml_raw$global_metadata$bounding_coordinates$north,
							xml_raw$global_metadata$bounding_coordinates$south)))
